...
import { analyzeUrl, analyzeEmail } from './api.js';
...
export function ScannerView() {
  ...
  if (!inputValue.trim()) {
    alert('Please enter a URL or Email text before analyzing.');
    return;
  }

  let urlToScan = inputValue;

  if (scanType === 'URL') {
    // If the URL has no scheme, assume https://
    urlToScan = inputValue.trim();
  ...
  if (scanType === 'Email' && inputValue.length < 10) {
    alert('Email text is too short.');
    return;
  }
  ...
  try {
    const result = scanType === 'URL'
      ? await analyzeUrl(urlToScan)
      : await analyzeEmail({
          subject: emailSubject || null,
          body_text: inputValue,
        });
    ...
    <main className="scanner-hero">
        ...
        <>
            ...
            <div className="info-anchor">
                ...
                <div className="info-message">
                    Formal Languages (automata and machine learning) are used to check for known malicious patterns
                </div>
                )}
            </div>
            </div>
      ...
      {scanError && !loading && (
        <div className="error-box" style={{
            ...
            <strong>⚠ Server Unavailable.</strong> {scanError} server. Please try again later.
        </div>
      )}
      ...
      {scanResult && !loading && (
        <div className="result-box">
          ...
          <p>Likelihood this {scanType} is phishing</p>

          {scanResult.reasons && scanResult.reasons.length > 0 && (
            <div className="reasons-list">
                <h3>Why?</h3>
                <ul>
                {scanResult.reasons.slice(0, 8).map((reason, i) => (
                    <li key={i}>{reason}</li>
                ))}
                </ul>
            </div>
          ...
          {scanResult.matched_rules && scanResult.matched_rules.length > 0 && (
            <details className="matched-rules">
                <summary>Matched rules ({scanResult.matched_rules.length})</summary>
                <ul>
